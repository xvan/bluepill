...
struct Si5351RegSet exported_pllreg;
struct Si5351RegSet exported_msreg;
    ...
    enum si5351_pll pll_prog)
{
	...
	si5351_set_ms_source(clock, pll_prog);
}
 ...
 */
uint32_t
si5351_set_freq(uint32_t freq, uint32_t pll_freq, enum si5351_clock clock)
{
...
multisynth_recalc(uint32_t freq, uint32_t pll_freq, struct Si5351RegSet *reg)
{
	...
	/* R	ecalculate output frequency by fOUT = fIN / (a + b/c) */
	freq = (uint32_t) ((((uint64_t)pll_freq) * c) / (a * c + b));
